...
Chalk 的核心是：**在字符串外包裹 ANSI 转义码，终端解析这些码后改变后续输出的颜色/样式**。

1. **ANSI 转义码**：终端遵循 ECMA-48/ISO 6429，以 `\x1B[...m`（或 `\u001b[...m`）形式插入「设置颜色/加粗/重置」等指令，例如红色为 `\x1B[31m`，重置为 `\x1B[0m`。Chalk 在传入字符串前后拼接对应转义序列。
2. **链式调用**：每次调用 `chalk.red`、`chalk.bold` 等会返回一个「带新样式序列」的代理对象，继续链式时在已有序列后追加，最后转成字符串时统一输出「所有样式码 + 文本 + 重置码」。
3. **能力检测**：根据环境变量（如 `TERM`）或 `supportsColor` 判断当前是否支持颜色；不支持时可选地去掉转义码，只输出纯文本，避免乱码。
4. **True Color / 256 色**：除 16 色基础码外，可用 256 色索引或 24 位 RGB 的转义序列，Chalk 封装为 `chalk.hex('#ff0000')`、`chalk.rgb(255,0,0)` 等。
