...
Semver 的核心是：**按 [semver.org](https://semver.org/) 规范把版本字符串解析成「主.次.修订」及可选的预发布/构建标识，再按规范定义的比较规则做大小判断与范围匹配**。

1. **解析**：将 `"1.2.3-beta.0"` 等字符串拆成 `major`、`minor`、`patch`、`prerelease`、`build` 等字段；不合法（如缺数字、非法字符）则返回 null 或抛错。
2. **比较规则**：先比 major，再 minor，再 patch；预发布版本（如 `1.0.0-alpha`）小于同号的正式版（`1.0.0`）；预发布之间按标识符的字典序或数字比较。
3. **范围（range）**：`^1.2.0`、`~1.2.0`、`>=1.0.0 <2.0.0` 等被解析成「满足条件的版本集合」；`satisfies(version, range)` 判断版本是否落在该集合内，与 npm 的依赖解析规则一致。
4. **coerce**：从 `"v1.2.3"`、`"1.2.3.4"` 等非规范字符串中「尽量」抽出合法的主.次.修订，便于兼容用户输入或 tag 名。
