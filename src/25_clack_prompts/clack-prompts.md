...
9. [展示与日志](#展示与日志)
10. [通用选项与进阶](#通用选项与进阶)
11. [与 @inquirer/prompts 的区别](#与-inquirerprompts-的区别)
12. [最佳实践与参考](#最佳实践与参考)
...
@clack/prompts 的核心是：**在 TTY 下监听键盘输入（readline/raw mode），根据当前「问题类型」更新光标所在行的内容（选项高亮、输入字符），用户确认后把结果 resolve 给调用方；用 \r 或清行实现原地刷新**。

1. **输入监听**：通过 Node 的 readline 或设置 raw mode 监听按键；区分普通字符、回车、上下键、Ctrl+C 等，按当前组件类型处理（文本输入追加字符、选择类上下移动高亮）。
2. **原地刷新**：每次状态变化（输入字符、高亮项变化）时，重算当前行要显示的内容（题干 + 选项/输入框），用 `\r` 回到行首再写或先清行再写，实现「同一行更新」而不刷屏。
3. **组件与 Promise**：每个 `text()`、`select()` 等返回 Promise；内部在用户确认（回车）或取消（Ctrl+C）时 resolve 结果或 reject，调用方 await 即可拿到答案。
4. **与 @clack/core**：底层由 @clack/core 提供「框线、引导线、样式」等绘制，prompts 在其上封装「问题类型 + 输入逻辑」，形成统一风格的交互 UI。
