...
SVGO 的核心是：**将 SVG 源码解析成 AST（或 DOM 树），按一系列「插件」规则对节点做删除、合并、简化，再序列化回 SVG 字符串，在尽量保持视觉一致的前提下减小体积**。

1. **解析**：把 SVG 字符串解析成结构化树（标签、属性、子节点）；便于按节点类型、属性名做规则处理。
2. **插件**：每个优化项是一个插件（如 removeViewBox、removeComments、mergePaths）；插件遍历节点，删除冗余（注释、metadata、默认属性）、合并路径、简化变换与数值精度，或重写 id 避免冲突。
3. **序列化**：把处理后的树再转回 SVG 字符串；可配置缩进、精度等，输出更短或更可读。
4. **无损与有损**：部分插件是「无损」（只删不影响显示的内容）；部分可能微调 path 等，属「有损」，需根据需求开关插件。
