...
Commander.js 的核心是：**解析 `process.argv` → 按「命令/选项/参数」拆解 → 匹配到注册的 Command 后执行对应 action**。

1. **解析 argv**：读取 `process.argv`（如 `['node', 'cli.js', 'create', 'my-app', '--force']`），去掉前两位得到子命令与参数列表。
2. **命令匹配**：程序维护一棵「命令树」（根 + 子命令），按 argv 中的 token 依次匹配；匹配到某 Command 且无更多子命令时，该命令即为本次要执行的。
3. **选项解析**：在匹配到的命令上，解析 `--option`、`-o`、`--key=value` 等，按 Option 定义做类型转换和默认值，得到 `options` 对象。
4. **参数收集**：按 Command 上定义的 `argument`（如 `<name>`、`[dest]`）从剩余 argv 中按顺序取值，传入 action。
5. **执行 action**：调用该 Command 的 `action(fn)` 注册的回调，传入解析好的参数和 options；若 action 返回 Promise，会等待完成后再结束进程（可选）。
