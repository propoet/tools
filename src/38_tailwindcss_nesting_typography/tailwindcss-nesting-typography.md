...
1. [概述](#概述)
2. [原理：PostCSS 管线与嵌套/排版](#原理postcss-管线与嵌套排版)
3. [@tailwindcss/nesting](#tailwindcssnesting)
...
**核心思路**：Tailwind 和嵌套/排版都在 **PostCSS 管线**里执行；顺序决定谁先处理。嵌套要先于 Tailwind，这样 `@apply`、`@screen` 等才能在已展开的嵌套结构上正确生效；typography 是 Tailwind 的插件，在 Tailwind 展开 utility 时注入 `prose` 等类对应的样式。

- **嵌套插件**：@tailwindcss/nesting 包装 postcss-nested 或 postcss-nesting，把「嵌套块」展开成扁平选择器；必须放在 tailwindcss 之前，否则 Tailwind 的自定义 at-rule（如 `@apply`）可能被嵌套插件误解析；插件内部会识别并跳过 Tailwind 专用语法。
- **Typography 插件**：作为 Tailwind 的 plugin，在生成 utility 时增加 `prose`、`prose-lg` 等类，对应一整套针对正文（标题、段落、列表、代码块等）的默认样式；不涉及 AST 嵌套，只是「多生成一批 utility」。
