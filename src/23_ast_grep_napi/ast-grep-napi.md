...
2. [原理：如何做基于 AST 的搜索与修改](#原理如何做基于-ast-的搜索与修改)
3. [安装与引入](#安装与引入)
4. [核心概念](#核心概念)
5. [主入口函数](#主入口函数)
6. [SgRoot 与 SgNode](#sgroot-与-sgnode)
7. [搜索与匹配](#搜索与匹配)
8. [遍历与检查](#遍历与检查)
9. [代码修改（Edit）](#代码修改edit)
10. [多文件搜索 findInFiles](#多文件搜索-findinfiles)
11. [类型与配置](#类型与配置)
...
@ast-grep/napi 的核心是：**把源码解析成 AST（抽象语法树），再按「模式/规则」在树上做结构匹配与遍历，匹配到的节点可读取、可替换，从而实现「按语法结构」的搜索、检查与重写**。

1. **解析**：用内置解析器（如 Tree-sitter 或类似）把源代码串解析成 AST，每个节点有类型、子节点、文本范围；支持多种语言（JS/TS/HTML/CSS 等）。
2. **匹配**：用模式（如 `console.log($A)`）或规则在 AST 上做结构匹配，$A 等占位符绑定到具体子节点；匹配到后可在回调里读取节点内容、收集信息。
3. **遍历**：可按节点类型遍历整棵树（如所有函数声明、所有调用表达式），不依赖「字符串正则」，避免误匹配或漏匹配。
4. **修改**：对匹配到的节点可生成「编辑」（如替换文本、插入/删除），再按范围应用回源码字符串，实现自动化重写或 codemod。
